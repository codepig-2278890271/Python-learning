...
# ------------------------------------------------------------
# 【try/except 详解】try = "试一下"，except = "出错了就接住"
#
# 执行流程：
#   a / b 成功 → 继续执行 try 块里剩余的代码 → 跳过 except
#   a / b 失败 → 立刻跳到 except，不再执行 try 里后续的代码
#
#   try:
#       result = a / b          ← 这里炸了（b=0）
#       print(f"{a}/{b}=...")   ← 这行被跳过！不会执行
#   except ZeroDivisionError:   ← 只有"除以零"类型的错误才被接住
#       ...                     ← 接住后执行这里的补救代码
#
# 如果不用 try/except，a / b 会直接把程序炸崩溃（Traceback 报错退出）
# ------------------------------------------------------------
try:
    result = a / b
    print(f"{a} / {b} = {result}")
